...
<div class="contact-us">
    <form action="{{url('notice/'.$notice->id)}}" method="post">
      @csrf
      @method("PUT")
        <input type="text" value="{{$notice->title}}" placeholder="Title" required name="title">
        <input type="text" value="{{$notice->description}}" placeholder="Description" required name="description">
        <input type="date" placeholder="Date" required name="date" value="{{$notice->date}}">
        <button type="submit">Edit and Update Notice</button>
    </form>
</div>
